...
def coinflip_callback(call):
 try:
    ...
    if(call.from_user.first_name==user.name.name): #avoid other people in groups to be able to click your buttons
     coinflip=["Testa","Croce"]
     if (user.name.name.name.money==0):
        ...
        user.name.name.money=user.name.name.money*2
        if str(user.name.name.money).endswith(".0"):
            user.name.name.money=int(user.name.name.money)
        try:
         bot.edit_message_text(text=call.from_user.first_name+" ha vinto "+str(user.name.name.money)+" euro", message_id=call.message.message_id,chat_id=call.message.chat.id)
         money_dict[call.message.chat.id][call.from_user.first_name]=user.name.name.money
        except Exception as e:
         if "400" in str(e):
             splitted_text=str(user.name.name.money)
        ...
        money_dict[call.message.chat.id][call.from_user.first_name]-=user.name.name.money
     user.name.name.money=0 #reset to avoid betting from old buttons
 except Exception as e:
     if e == ValueError:
...
def Testa_o_Croce(message):
   try:
    user = User()
    user_dict[message.chat.id] = user
    user.name.name.money=float(message.text)
    user.name.name=message.from_user.first_name
