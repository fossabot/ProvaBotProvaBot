...
for x in lista_cartelle:
    if os.path.exists(bot_path + x) == False:
        print("Manca la cartella " + x.replace("/", "") +
              ", la creo inserendoci un file .jpg vuoto")
        os.makedirs(bot_path + x)
        if os.path.isfile(bot_path + "/nope.jpg") == False:
            urllib.request.urlretrieve(
                "http://www.neacuho.org/resource/resmgr/EBoard_Photos/2015-2016/No_Image.jpg", "nope.jpg")
        shutil.copy2(bot_path + "/nope.jpg", bot_path + x)
lista_playmate = [f for f in listdir(
    bot_path + "/playmates") if isfile(join(bot_path + "/playmates", f))]
lista_cibo = [f for f in listdir(
    bot_path + "/cibo") if isfile(join(bot_path + "/cibo", f))]
lista_strisce = [f for f in listdir(
    bot_path + "/strisce") if isfile(join(bot_path + "/strisce", f))]
lista_xkcd = [f for f in listdir(
    bot_path + "/xkcd") if isfile(join(bot_path + "/xkcd", f))]
lista_videoporno = [f for f in listdir(
    bot_path + "/videoporno") if isfile(join(bot_path + "/videoporno", f))]
...
def coinflip_callback(call):
    try:
        ...
        # avoid other people in groups to be able to click your buttons
        if(call.from_user.first_name == user.name):
            coinflip = ["Testa", "Croce"]
            if (user.money == 0):
                bot.edit_message_text(text="Volevi aver vinto qualcosa eh? Invece no",
                ...
                try:
                    bot.edit_message_text(text=call.from_user.first_name + " ha vinto " + str(
                        user.money) + " euro", message_id=call.message.message_id, chat_id=call.message.chat.id)
                    ...
                    if "400" in str(e):
                        ...
                        print(
                            str(e) + " in funzione coinflip durante l'invio del messaggio all'utente ")
            else:
                bot.edit_message_text(text=call.from_user.first_name + " ha perso tutto",
                                      message_id=call.message.message_id, chat_id=call.message.chat.id)
            user.money = 0  # reset to avoid betting from old buttons
    except Exception as e:
        if e == ValueError:
            bot.edit_message_text(text=call.from_user.first_name + " ha inserito qualcosa che non doveva",
                                  message_id=call.message.message_id, chat_id=call.message.chat.id)
        else:
...
def informa(message):
    msg = bot.send_message(
        message.chat.id, "Inserisci un messaggio da criptare")
    ...
    try:
        ...
        if "400" in str(e):
            splitted_text = util.split_string(
                cipher_text.decode(encoding='UTF-8'), 3000)
            for text in splitted_text:
...
def ottieni_key(message):
    ...
    msg = bot.send_message(
        message.chat.id, 'Invia la key ricevuta insieme al messaggio criptato')
...
def decripta_messaggio(message):
    try:
        ...
        user.key = key
        plain_text = Fernet(user.key.encode(encoding='UTF-8')
                            ).decrypt(user.encmessage.encode(encoding='UTF-8'))
        bot.send_message(
            message.chat.id, "Il messaggio decriptato è il seguente:")
        try:
            bot.send_message(
                message.chat.id, plain_text.decode(encoding='UTF-8'))
        except Exception as e:
            if "400" in str(e):
                splitted_text = util.split_string(
                    plain_text.decode(encoding='UTF-8'))
                for text in splitted_text:
                    bot.send_message(message.chat.id, text)
            else:
        ...
        risposta(
            message, 'Si è verificato un errore, sicuro di aver copiato bene il messaggio?')
        print(str(e) + " in funzione decripta_messaggio")
...
def Testa_o_Croce(message):
    try:
        ...
        markup.row(testa)
        markup.row(croce)
        bot.send_message(chat_id=message.chat.id, text="Testa o Croce?",
                         reply_markup=markup, parse_mode="Markdown")
    except ValueError:
        risposta(
            message, "Devi inserire un numero, non lettere! Riprova da capo con /coinflip")
...
def invia_playmate(message):
    try:
        ...
        bot.send_photo(message.chat.id, open(
            bot_path + "/playmates/" + nome_file, 'rb'))
...
def invia_video_porno(message):
    ...
    try:
        bot.send_video(message.chat.id, open(
            bot_path + "/videoporno/" + random.choice(lista_videoporno), 'rb'))
...
def download_motherless(message):
    try:
        ...
        if ((" " not in message.text)):
            risposta(
                message, "Il tuo messaggio è vuoto, devi fornire un video da scaricare")
        else:
            messaggio = message.text.split(" ")[1]
            ...
            urllib.request.urlretrieve("http://cdn4.videos.motherlessmedia.com/videos/" +
                                       messaggio + ".mp4", bot_path + file_identifier + ".mp4")
            bot.send_chat_action(message.chat.id, 'upload_video')
            try:
                bot.send_video(message.chat.id, open(
                    bot_path + file_identifier + ".mp4", "rb"))
            except requests.exceptions.ChunkedEncodingError:
                print("ChunkedEncodingError in pornvid")
                risposta(message, "Si è verificato un errore, riprova")
            except telebot.apihelper.ApiException:
...
def download(message):
    try:
        ...
        if " " not in message.text:
            risposta(
                message, "Il tuo messaggio è vuoto, devi fornire un link da scaricare")
        else:
            messaggio = message.text.split(" ")[1]
            if (urllib.parse.urlparse(messaggio) == True):
                estensione = messaggio.split(".")[-1]
                file_identifier = str(message.chat.id) + str(int(time.time()))
                file_size = (urllib.request.urlopen(
                    messaggio)).headers["Content-Length"]
    ...
    try:
        ...
        if messaggio == (""):
            ...
            if y <= len(link_usabili):
                ...
                risposta(
                    message, "L'indice specificato è maggiore di quanti sono i link trovati")
            markup = types.ReplyKeyboardMarkup(one_time_keyboard=True)
            for x in range(0, len(link_usabili)):
                markup.add(
                    str("/pornsrc " + messaggio_keyboard + " " + str(x)))
...
def invia_cibo(message):
    ...
    try:
        bot.send_photo(message.chat.id, open(
            bot_path + "/cibo/" + random.choice(lista_cibo), 'rb'))
    except Exception as e:
        risposta(message, "Si è verificato un errore, riprova")
        print(str(e) + " in cibo")
...
def invia_striscia(message):
    ...
    try:
        bot.send_photo(message.chat.id, open(
            bot_path + "/strisce/" + random.choice(lista_strisce), 'rb'))
    except Exception as e:
        risposta(message, "Si è verificato un errore, riprova")
...
def invia_xkcd(message):
    ...
    try:
        bot.send_photo(message.chat.id, open(
            bot_path + "/xkcd/" + random.choice(lista_xkcd), 'rb'))
    except Exception as e:
        risposta(message, "Si è verificato un errore, riprova")
        print(str(e) + " in xkcd")
...
def insulta(message):
    try:
        ...
        if messaggio != "":
            ...
            risposta(
                message, "aggiungi un nome o qualcuno da insultare dopo il comando(ad esempio /insulta mario), coglione!")
    except Exception as e:
